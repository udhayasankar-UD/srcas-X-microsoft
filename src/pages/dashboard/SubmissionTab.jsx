...
export default function SubmissionTab({ hasTeam, teamData, submissions, setSubmissions }) {
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      ...
      <div className="dash-card" style={card()}>
        {step === 0 && (
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
              <label style={{ fontSize:13, fontWeight:600, color:'#374151' }}>Project Description (Max 500 words)</label>
              <textarea value={form.description} onChange={set('description')} placeholder="Describe your solution..." rows={6}
                style={{ padding:'12px 14px', borderRadius:10, border:'1.5px solid #e5e7eb', fontSize:13, color:'#111', outline:'none', resize:'vertical', fontFamily:'inherit' }}/>
            </div>
          </div>
        )}
        {step === 1 && (
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
              <label style={{ fontSize:13, fontWeight:600, color:'#374151' }}>Upload Presentation Deck (PDF only, Max 20MB)</label>
              <input type="file" accept=".pdf" onChange={e => setForm({...form, pdf: e.target.files[0]})}
                style={{ padding:'10px 13px', borderRadius:10, border:'1.5px solid #e5e7eb', fontSize:13, color:'#111' }}/>
            </div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={handleSaveDraft} style={{ padding:'10px 18px', borderRadius:10, border:'1.5px solid #e5e7eb', background:'#f9fafb', fontSize:13, fontWeight:600, cursor:'pointer', color:'#374151' }}>
            Save Progress
          </button>
          {step < STEPS.length-1
            ...
            : <button disabled={submitting} onClick={handleSubmit} style={{ padding:'10px 28px', borderRadius:10, border:'none', background:'linear-gradient(135deg,#4C9F38,#3d8a2e)', color:'#fff', fontSize:13, fontWeight:700, cursor:submitting?'not-allowed':'pointer', boxShadow:'0 4px 14px rgba(76,159,56,0.3)', opacity: submitting?0.7:1 }}>
